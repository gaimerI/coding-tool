...
Prism.languages.mccommand = {
	...
	'variable': [
		/\b(?:reset|black|dark_blue|dark_green|dark_aqua|dark_red|dark_purple|gold|gray|dark_gray|blue|green|aqua|red|light_purple|yellow|white|minecoin_gold|material_quartz|material_iron|material_netherite|material_redstone|material_copper|material_gold|material_emerald|material_diamond|material_lapis|material_amethyst|material_resin)\b/,
		/\b(?:world_surface|motion_blocking|motion_blocking_no_leaves|ocean_floor)\b/
	],
	
	'builtin': [
		/\b(?:eyes|feet)\b/,
		/\b(?:survival|creative|adventure|spectator)\b/
	],
	
	'target-selector': {
		pattern: /@(?:p|a|r|e|s|n|initiator)(?:\[[^\]]*\])?|\b[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\b/i,
        greedy: true,
        inside: {
            'selector-type': {
                pattern:/^@(?:p|a|r|e|s|n|initiator)/,
                alias:'keyword'
             },
             'selector-arguments': {
                pattern: /\[[^\]]*\]/,
     ...
     'block-predicate':{pattern:/(?:#)?(?:[a-z0-9_.-]+:)?[a-z0-9_.-]+(?:\[[^\]]*\])?(?:\{[^}]*\})?/,greedy:true,inside:{'tag-prefix':{pattern:/^#/,alias:'important'},'namespace':{pattern:/^[a-z0-9_.-]+:/,alias:'namespace'},'block-id':{pattern:/[a-z0-9_.-]+/,alias:'class-name'},'block-states':{pattern:/\[[^\]]*\]/,inside:{'punctuation':/[\[\]=,]/,'property':/\b[a-z0-9_.-]+\b/i}},'nbt-data':{pattern:/\{[^}]*\}/,inside:{'punctuation':/[{}:,]/,'property':/\b[a-z0-9_.-]+\b/i,'string':/"([^"\\]*(\\.[^"\\]*)*)"/,'number':/-?\d+(\.\d+)?/}}}}};
