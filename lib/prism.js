...
Prism.languages.mccommand = {
    // Command names (fixed so /tp works, 0/tp doesn't)
    'keyword': {
    ...
    // Block predicates with sub-highlighting
    'block-predicate': {
	...
	'target-selector': {
		// Matches either @p, @a, @r, @e, @s with optional [arguments]
        // OR matches a UUID
        pattern: /@(?:p|a|r|e|s)(?:\[[^\]]*\])?|\b[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\b/i,
        greedy: true,
        inside: {
            'selector-type': {
                pattern: /^@(?:p|a|r|e|s)/,
	...
	'range': {
		// Matches things like: 0.. , 0..5 , ..-6.7 , -2.5..3.14
