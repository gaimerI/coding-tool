...
// PrismJS language: Strudel (pattern-based live coding DSL)
(function (Prism) {
  Prism.languages.strudel = {
	// Metadata lines like @title, @by, etc.
	'metadata': {
		pattern: /^@\w+.*/m,
		alias: ['keyword']  // styled as comment but keyword-colored
	},
    // // comments and /* ... */ just in case
    'comment': [
      {
        pattern: /\/\/.*/,
        greedy: true
    ...
    // Channel / pane labels like `$:` or `$_:`
    'channel': {
      pattern: /^\s*\$[\w$]*\s*:/m,
      alias: ['symbol', 'important']
    },

    // Strings (single or double). Keep it simple & greedy so pattern text is handled.
    'string': {
      pattern: /"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'/,
      greedy: true
    },

    // Numbers (ints, floats, fractions like 1/4 split into number + operator + number)
    'number': /\b(?:0x[\da-fA-F]+|\d+(?:\.\d+)?)\b/,

    // Top-level function calls: foo(...)  (e.g., setcps(0.5), hubda('...'))
    // We color the identifier as 'function' when immediately followed by '('.
    'function-call': {
      // start of line, after punctuation, or whitespace -> name( ... )
      pattern: /(?:^|[^\w.])([a-zA-Z_]\w*)(?=\s*\()/m,
      inside: {
        'function': /[a-zA-Z_]\w*/
      }
    },

    // Method calls: .anything(...) — your requested simplified rule
    'method': {
      pattern: /\.[a-zA-Z_]\w*(?=\s*\()/,
      alias: 'function'
    },

    // Built-ins / known identifiers (kept lightweight; add/remove as you like)
    'builtin': /\b(?:setcps|setcpm|samples|hubda|spagda|speechda|register|all|when|swing|bank|room|crush|coarse|press|decay|dec|dist|off|hpf|hpq|fm|s|n|scale|scaleTranspose|euclid|slow|fast|fit|slice|run|iter|speed|begin|range)\b/,

    // Bank/sample keys like sd:1 cp:0 bd: etc. (left side highlighted)
    'key': {
      pattern: /\b[a-zA-Z_]\w*(?=:)(?=:[^:\s])/,
      alias: 'property'
    },

    // Operators and punctuation typical in pattern DSLs
